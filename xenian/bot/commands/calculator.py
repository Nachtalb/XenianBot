...
class Calculator(BaseCommand):
        ...
        self.commands = [
            {
                "title": "Calculator",
                "description": (
                    "Solve equations you send me, to get a full list of supported math functions use /maths "
                    "(PRIVATE CHAT ONLY)"
                ),
    ...
    @run_async
    def calcualate(self, bot: Bot, update: Update, equation: str = ""):
        """Calculate
        ...
        try:
            ...
            try:
                reply = reply_template.format(message=message, equation=equation, result=result)
    ...
    def calcualate_command(self, bot: Bot, update: Update, args: list = []):
        """Calculate
    ...
    def maths(self, bot: Bot, update: Update):
        ...
        for math_func in self.safe_dict.values():
            ...
            math_list += "\n{seperator}\n{description}".format(seperator="-" * 20, description=doc)
