...
class Anime(BaseCommand):
    ...
    def toggle_mode(self, bot: Bot, update: Update):
        ...
        self.gif_save_mode.update({'chat_id': chat_id},
                                  {'chat_id': chat_id, 'mode': new_mode},
                                  upsert=True)
        update.message.reply_text('GIF save mode turned `%s`' % ('on' if new_mode else 'off'),
    ...
    def save_gif(self, bot: Bot, update: Update):
        ...
        Args:
            ...
            update (:obj:`telegram.update.Update`): Telegram Api Update Object
        """
        video = (update.message.document or update.message.video or update.message.reply_to_message.document or
