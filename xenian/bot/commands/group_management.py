...
class GroupManager(BaseCommand):
    ...
    def __init__(self):
        self.commands = [
            ...
            },
            {
                "title": "Strike",
                "description": (
                    "Warn a user, after 3 warnings he get banned. Reply to one of his messages with this "
                    "command (Group Only)"
                ),
                "command": self.warn,
                "options": {
            ...
            },
            {
                "title": "Kick",
                "description": (
                    "Kick a user for 10 min or give a specific amount of time (in min) between 30sec "
            ...
            },
            {
                ...
                "description": (
                    "Delete a message from a user and warn them. Reply to one of his messages with this "
                    "command (Group Only)"
                ),
                "command": self.delete,
                "options": {
            ...
            },
            {
                ...
                "description": (
                    "Remove all warnings from a User. Reply to one of his messages with this command (Group Only)"
                ),
                "command": self.unwarn,
                "options": {
                    "filters": (
            ...
            },
            {
                ...
                "options": {"filters": (Filters.group & filters.user_group_admin)},
            },
            {
                "title": "Remove Rules",
    ...
    def kick(self, bot: Bot, update: Update, args: list = []):
        ...
        bot.kick_chat_member(chat_id=chat_id, user_id=wanted_user.id, until_date=kick_until)
            ...
            callback=(
                lambda bot, job: bot.send_message(
                    ...
                    "{wanted_user} was kicked {time} min ago and can now join again".format(
                        time=time, wanted_user=get_user_link(wanted_user)
                    ),
        ...
        bot.send_message(
            ...
            text="{wanted_user} was kicked for {time} min by {from_user}".format(
                time=time, from_user=get_user_link(from_user), wanted_user=get_user_link(wanted_user)
            ),
            parse_mode=ParseMode.MARKDOWN,
        )
    ...
    def ban(self, bot: Bot, update: Update):
        ...
        bot.kick_chat_member(chat_id=chat_id, user_id=wanted_user.id, until_date=now + datetime.timedelta(seconds=5))
        bot.send_message(
            chat_id=chat_id,
            text="{wanted_user} was banned by {from_user}".format(
                from_user=get_user_link(from_user), wanted_user=get_user_link(wanted_user)
            ),
            parse_mode=ParseMode.MARKDOWN,
        )
    ...
    def warn(self, bot: Bot, update: Update, wanted_user: User | None = None):
        """Strike a user
    ...
    def unwarn(self, bot: Bot, update: Update, wanted_user: User | None = None):
        ...
        if not group_data.get(chat_id, None) or not group_data[chat_id].get(wanted_user.id, None):
            bot.send_message(
                chat_id=chat_id,
                text="{wanted_user} was never warned.".format(wanted_user=get_user_link(wanted_user)),
                parse_mode=ParseMode.MARKDOWN,
            )
            return
    ...
    def rules_remove(self, bot: Bot, update: Update):
        ...
        if not group_data.get(chat_id, None) or not group_data[chat_id].get("rules", None):
            bot.send_message(chat_id=chat_id, text="This group has no rules defined, use /rules_define to add them.")
            return
        ...
        bot.send_message(
            chat_id=chat_id,
            text="{user} has set removed the groups rules.".format(user=get_user_link(from_user)),
            parse_mode=ParseMode.MARKDOWN,
        )
    ...
    def rules(self, bot: Bot, update: Update):
        ...
        bot.send_message(chat_id=chat_id, text=group_data[chat_id]["rules"], parse_mode=ParseMode.MARKDOWN)
