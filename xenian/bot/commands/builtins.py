...
class Builtins(BaseCommand):
    ...
    def run_commands(self, bot: Bot, update: Update, args: list = []):
        ...
        for command_class in BaseCommand.all_commands:
            ...
            for command in [
                cmd for cmd in command_class.commands if cmd["handler"] == CommandHandler and not cmd["hidden"]
            ]:
            ...
            for command in [
                cmd for cmd in command_class.commands if cmd["handler"] == MessageHandler and not cmd["hidden"]
            ]:
        ...
        if "raw" in args:
            ...
            reply = render_template(
                "commands.html.mako", direct_commands=direct_commands, indirect_commands=indirect_commands
            )
    ...
    def contribute(self, bot: Bot, update: Update):
        ...
        admin_text = "Contribution form {user}: {text}".format(
            user=get_user_link(update.message.from_user), text=text, parse_mode=ParseMode.MARKDOWN
        )
