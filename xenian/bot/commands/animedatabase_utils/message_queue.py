...
class MessageQueue:
    ...
    def __init__(self, total: int, message: Message, group_size: int = 1):
        self.total = total
    ...
    def report(self, error: PostError | None = None):
        if error:
    ...
    def finished(self):
        ...
        if lines:
            self.message.reply_text(
                "\n".join(lines), reply_to_message_id=self.message.message_id, disable_web_page_preview=True
            )
